...
var XliffEditor = (
    function () {
        ...
        function XliffEditor(place, givenOptions) {
            // Determine effective options based on given values and defaults.
            var options = {}
            var defaults = XliffEditor.defaults
            ...
            wrapper.innerHTML =
                ...
                '<div class="XliffEditor-gutter"><div class="XliffEditor-gutter-text"></div></div>' +
                ...
                '<div class="XliffEditor-lines"><div style="position: relative">' +
                '<pre class="XliffEditor-cursor">&#160;</pre>' + // Absolutely positioned blinky cursor
            ...
            var measure = code.firstChild
            var mover = measure.nextSibling
            var gutter = mover.firstChild
            var gutterText = gutter.firstChild
            var inputDiv = gutter.nextSibling
            var input = inputDiv.firstChild
            var lineSpace = inputDiv.nextSibling.firstChild
            var cursor = lineSpace.firstChild
            var lineDiv = cursor.nextSibling
			...
			// Somewhat hacky (delayed prepareInput) to work around browser issues.
            operation(function () {
            ...
            // The instance object that we'll return. 
			// Mostly calls out to local functions in the CodeMirror function. 
			// Some do some extra range checking and/or clipping. 
			// operation is used to wrap the call so that changes it makes are tracked, 
			// and the display is updated afterwards.
            var instance = {
                getValue: getValue,
            ...
            function setValue(code) {
                history = null
                var top = { line: 0, ch: 0 }
                updateLines(top, { line: lines.length - 1, ch: lines[lines.length - 1].text.length }, splitLines(code), top, top)
                history = new History()
            }
            ...
            function loadMode() {
                mode = XliffEditor.getMode(options, options.mode)
                for (var i = 0, l = lines.length; i < l; ++i) {
            ...
            function markText(from, to, className) {
                ...
                function add(line, from, to, className) {
                    var line = lines[line]
                    var mark = line.addMark(from, to, className)
                    mark.line = line
                    accum.push(mark)
                }
                ...
                return function () {
                    ...
                    for (var i = 0; i < accum.length; ++i) {
                        var mark = accum[i]
                        var found = indexOf(lines, mark.line)
            ...
            function charWidth() {
                return (measure.firstChild.offsetWidth || 320) / 40
            }
            ...
            function matchBrackets(autoclear) {
                ...
                for (var i = head.line, e = forward ? Math.min(i + 50, lines.length) : Math.max(0, i - 50); i != e; i += d) {
                    ...
                    if (found) {
                        var style = found.match ? 'XliffEditor-matchingbracket' : 'XliffEditor-nonmatchingbracket'
                        var one = markText({ line: head.line, ch: pos }, { line: head.line, ch: pos + 1 }, style)
						var two = markText({ line: i, ch: found.pos }, { line: i, ch: found.pos + 1 }, style)
            ...
            function onContextMenu(e) {
                ...
                var oldCSS = input.style.cssText
                input.style.cssText =
                    'position: fixed; width: 30px; height: 30px; top: ' + (e.pageY() - 1) +
                    'px; left: ' + (e.pageX() - 1) + 'px; z-index: 1000; background: white; ' +
                    'border-width: 0; outline: none; overflow: hidden;'
                var val = input.value = getSelection()
                input.focus()
                ...
                if (typeof query != 'string') { // Regexp match
                    this.matches = function (reverse, pos) {
                        if (reverse) {
                            ...
                            var match = line.match(query)
                            var start = match && pos.ch + line.indexOf(match[0])
                        }
                        if (match) {
                            return {
            ...
            SearchCursor.prototype = {
                ...
                find: function (reverse) {
                    var self = this
                    var pos = clipPos(reverse ? this.pos.from : this.pos.to)
        ...
        XliffEditor.defineMode = function (name, mode) {
            if (!XliffEditor.defaults.mode && name != 'null') {
                XliffEditor.defaults.mode = name
            }
            modes[name] = mode
        }

        XliffEditor.defineMIME = function (mime, spec) {
            mimeModes[mime] = spec
        }

        XliffEditor.getMode = function (options, spec) {
            ...
            if (!mfactory) {
                ...
                return XliffEditor.getMode(options, 'text/plain')
            }
            return mfactory(options, config)
        }

        XliffEditor.listModes = function () {
            var list = []
            for (var m in modes) {
        ...
        XliffEditor.listMIMEs = function () {
            var list = []
        ...
        XliffEditor.fromTextArea = function (textarea, options) {
            if (!options) {
                options = {}
            }
            ...
            var instance = XliffEditor(function (node) {
        ...
        XliffEditor.startState = startState

        function startState(mode, a1, a2) {
            return mode.startState ? mode.startState(a1, a2) : true
        }

        XliffEditor.copyState = copyState
        ...
        XliffEditor.defineMode('null', function () {
            return { token: function (stream) { stream.skipToEnd() } }
        })

        XliffEditor.defineMIME('text/plain', 'null')
